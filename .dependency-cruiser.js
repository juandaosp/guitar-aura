...
module.exports = {
  forbidden: [
    ...
    },
    {
      ...
      severity: 'error',
      comment: 'Los componentes de UI atómicos (shadcn, etc.) no deben acoplarse a la lógica de negocio (store o hooks locales).',
      from: { path: '^components/ui' },
      to: { path: '^(store|hooks|app)', pathNot: '^hooks/use-mobile' }
    },
    {
      name: 'no-orphans',
      comment:
      ...
      from: {
        ...
        pathNot: [
          ...
          '^app/.*(page|layout|loading|error|not-found)\\.tsx$',  // Ignore Next.js pages, layouts, errors, loading states
          '^next-env\\.d\\.ts$'      // Ignore global types
        ]
      },
      to: {},
    },
    ...
    },
    {
      ...
      from: {
        path: '^(app|lib|components|hooks|mocks|types)',
    ...
    {
      name: 'types-cannot-import-logic',
      severity: 'error',
      comment: 'Los archivos de definición de tipos (.ts de definición) no deben importar componentes o stores activos.',
      from: { path: '^types/' },
      to: { path: '^(components|store|app)' }
    }
  ],
  options: {
...
// generated: dependency-cruiser@17.4.3 on 2026-06-12T05:28:57.640Z
